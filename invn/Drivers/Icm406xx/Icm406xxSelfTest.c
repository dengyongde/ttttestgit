...
static int run_gyro_self_test(struct inv_icm406xx * s, int * result);
static int run_accel_self_test(struct inv_icm406xx * s, int * result);
static int average_sensor_output(struct inv_icm406xx * s, int sensor, int self_test_config, int32_t sensor_result[3]);
...
static int set_user_offset_regs(struct inv_icm406xx * s, uint8_t sensor);
static int reg_to_accel_fsr(ICM406XX_ACCEL_CONFIG0_FS_SEL_t reg);
static int reg_to_gyro_fsr(ICM406XX_GYRO_CONFIG0_FS_SEL_t reg);


int inv_icm406xx_run_selftest(struct inv_icm406xx * s, int * result)
{
	int status = 0;
	...
	*result = 0;
	...
	if (s->st_result == 0) {
		...
		if ((status == 0) && (gyro_result == 1))
			status |= set_user_offset_regs(s, INV_ICM406XX_SENSOR_ON_MASK_GYRO);
		
		status |= run_accel_self_test(s, &accel_result);
		if ((status == 0) && (accel_result == 1))
			status |= set_user_offset_regs(s, INV_ICM406XX_SENSOR_ON_MASK_ACCEL);
...
int inv_icm406xx_get_st_bias(struct inv_icm406xx * s, int st_bias[6])
{
	int status = 0;
	int i;
...
int inv_icm406xx_set_st_bias(struct inv_icm406xx * s, const int st_bias[6])
{
	int status = 0;
	int i;
...
static int run_gyro_self_test(struct inv_icm406xx * s, int * result)
{
	...
	*result = 1;
	...
	status |= inv_icm406xx_read_reg(s, MPUREG_GYRO_CONFIG0, 1, &data);
	data &= ~BIT_GYRO_CONFIG0_FS_SEL_MASK;
	data &= ~BIT_GYRO_CONFIG0_ODR_MASK;
	data |= ST_GYRO_FSR;
	data |= ST_GYRO_ODR;
	status |= inv_icm406xx_write_reg(s, MPUREG_GYRO_CONFIG0, 1, &data);
	...
	status |= average_sensor_output(s, INV_ICM406XX_SENSOR_ON_MASK_GYRO, 0, STG_OFF);

	/* Enable self-test for each axis and read average gyro digital output 
	 * for each axis and store them as STG_ON_{x,y,z} in lsb */
	status |= average_sensor_output(s, INV_ICM406XX_SENSOR_ON_MASK_GYRO, 
	...
	status |= inv_icm406xx_read_reg(s, MPUREG_XG_ST_DATA_B1, 3, ST_code_regs);
	bank = 0;
	status |= inv_icm406xx_write_reg(s, MPUREG_REG_BANK_SEL, 1, &bank);
...
static int run_accel_self_test(struct inv_icm406xx * s, int * result)
{
	...
	*result = 1;
	...
	status |= inv_icm406xx_read_reg(s, MPUREG_ACCEL_CONFIG0, 1, &data);
	data &= ~BIT_ACCEL_CONFIG0_FS_SEL_MASK;
	data &= ~BIT_ACCEL_CONFIG0_ODR_MASK;
	data |= ST_ACCEL_FSR;
	data |= ST_ACCEL_ODR;
	status |= inv_icm406xx_write_reg(s, MPUREG_ACCEL_CONFIG0, 1, &data);
	...
	status |= average_sensor_output(s, INV_ICM406XX_SENSOR_ON_MASK_ACCEL, 0, STA_OFF);

	/* Enable self-test for each axis and read average gyro digital output 
	 * for each axis and store them as ST_ON_{x,y,z} in lsb x 1000 */
	status |= average_sensor_output(s, INV_ICM406XX_SENSOR_ON_MASK_ACCEL, 
	...
	bank = 2;
	status |= inv_icm406xx_write_reg(s, MPUREG_REG_BANK_SEL, 1, &bank);
	status |= inv_icm406xx_read_reg(s, MPUREG_XA_ST_DATA_B2, 3, ST_code_regs);
	bank = 0;
	status |= inv_icm406xx_write_reg(s, MPUREG_REG_BANK_SEL, 1, &bank);
...
static int average_sensor_output(struct inv_icm406xx * s, int sensor, int self_test_config, int32_t average[3])
{
	int status = 0;
	...
	if(sensor == INV_ICM406XX_SENSOR_ON_MASK_GYRO) {
		...
		/* Enable Gyro */
		status |= inv_icm406xx_read_reg(s, MPUREG_PWR_MGMT_0, 1, &pwr_mgmt_reg);
		pwr_mgmt_reg &= (uint8_t)~BIT_PWR_MGMT_0_GYRO_MODE_MASK;
		pwr_mgmt_reg |= (uint8_t)ICM406XX_PWR_MGMT_0_GYRO_MODE_LN;
		status |= inv_icm406xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &pwr_mgmt_reg);
	...
	if(self_test_config) {
		status |= inv_icm406xx_read_reg(s, MPUREG_SELF_TEST_CONFIG, 1, &self_test_config_reg);
		self_test_config_reg |= self_test_config; 
		status |= inv_icm406xx_write_reg(s, MPUREG_SELF_TEST_CONFIG, 1, &self_test_config_reg);
	...
	do {
		...
		if (int_status & BIT_INT_STATUS_DRDY) {
			...
			if (s->endianess_data == ICM406XX_INTF_CONFIG0_DATA_BIG_ENDIAN) {
...
static int save_settings(struct inv_icm406xx * s, struct recover_regs * saved_regs)
{
	...
	status |= inv_icm406xx_read_reg(s, MPUREG_INTF_CONFIG1, 1, &saved_regs->intf_config1);
	status |= inv_icm406xx_read_reg(s, MPUREG_PWR_MGMT_0, 1, &saved_regs->pwr_mgmt_0);
	status |= inv_icm406xx_read_reg(s, MPUREG_ACCEL_CONFIG0, 1, &saved_regs->accel_config0);
	status |= inv_icm406xx_read_reg(s, MPUREG_ACCEL_CONFIG1, 1, &saved_regs->accel_config1);
	status |= inv_icm406xx_read_reg(s, MPUREG_GYRO_CONFIG0, 1, &saved_regs->gyro_config0);
	status |= inv_icm406xx_read_reg(s, MPUREG_GYRO_CONFIG1, 1, &saved_regs->gyro_config1);
	status |= inv_icm406xx_read_reg(s, MPUREG_ACCEL_GYRO_CONFIG0, 1, &saved_regs->accel_gyro_config0);
	status |= inv_icm406xx_read_reg(s, MPUREG_FIFO_CONFIG1, 1, &saved_regs->fifo_config1);
	status |= inv_icm406xx_read_reg(s, MPUREG_SELF_TEST_CONFIG, 1, &saved_regs->self_test_config);
...
static int recover_settings(struct inv_icm406xx * s, const struct recover_regs * saved_regs)
{
	...
	status |= inv_icm406xx_write_reg(s, MPUREG_SELF_TEST_CONFIG, 1, &saved_regs->self_test_config);
	/*Restore gyro_dec2_m2_ord, gyro_ui_filt_ord_ind and gyro_ui_filt_bw_ind to previous values.*/
	status |= inv_icm406xx_write_reg(s, MPUREG_INTF_CONFIG1, 1, &saved_regs->intf_config1);
	status |= inv_icm406xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &saved_regs->pwr_mgmt_0);
	status |= inv_icm406xx_write_reg(s, MPUREG_ACCEL_CONFIG0, 1, &saved_regs->accel_config0);
	status |= inv_icm406xx_write_reg(s, MPUREG_ACCEL_CONFIG1, 1, &saved_regs->accel_config1);
	status |= inv_icm406xx_write_reg(s, MPUREG_GYRO_CONFIG0, 1, &saved_regs->gyro_config0);
	status |= inv_icm406xx_write_reg(s, MPUREG_GYRO_CONFIG1, 1, &saved_regs->gyro_config1);
	status |= inv_icm406xx_write_reg(s, MPUREG_FIFO_CONFIG1, 1, &saved_regs->fifo_config1);
	status |= inv_icm406xx_write_reg(s, MPUREG_ACCEL_GYRO_CONFIG0, 1, &saved_regs->accel_gyro_config0);
...
static int set_user_offset_regs(struct inv_icm406xx * s, uint8_t sensor)
{
	...
	if (sensor == INV_ICM406XX_SENSOR_ON_MASK_ACCEL) {
		 ...
		 */
		status |= inv_icm406xx_read_reg(s, MPUREG_OFFSET_USER_4_B4, 1, &data[0]); // Fetch gyro_z_offuser[8-11]
		...
		status |= inv_icm406xx_write_reg(s, MPUREG_OFFSET_USER_4_B4, 5, &data[0]);
		 ...
		 */
		status |= inv_icm406xx_read_reg(s, MPUREG_OFFSET_USER_4_B4, 1, &data[4]); // Fetch accel_x_offuser[8-11]
		...
		status |= inv_icm406xx_write_reg(s, MPUREG_OFFSET_USER_0_B4, 5, &data[0]);
